...
// Initialize Groq client with explicit API key (if available)
let groq = null;
if (process.env.GROQ_API_KEY) {
  groq = new Groq({ apiKey: process.env.GROQ_API_KEY });
} else {
  console.warn('GROQ_API_KEY not set in environment variables; Groq-powered AI features will be disabled.');
}
...
async function curateResources(searchData, subject) {
  ...
  try {
    ...
    if (!groq) throw new Error('GROQ_API_KEY not set; cannot call Groq API');
